...
class OrderController extends AbstractController
{
     ...
     */
    #[Route(path: '/resend-mail/status/{statusLog}', name: 'resendmail_status', methods: ['GET'])]
    public function resendMailStatus(OrderStatusLog $statusLog, Mailer $mailer, Request $request)
    {
        ...
        if ($mailer->sendMessage($order->getClient()->getEmail(), $orderStatusMail->getSubject(), $orderStatusMail->getTemplate(), $order)) {
            ...
            return $this->redirect(urldecode((string) $request->get('callback')));
        }
    }

    #[Route(path: '/detail/{id}', name: 'show', methods: ['GET'])]
    ...
    #[Route(path: '/capture/payement', name: 'capture', methods: ['GET'])]
