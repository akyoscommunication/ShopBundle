...
class CartService
{
	...
	public function getCart($token = null)
	{
		...
		$cartSession = ($this->session->getSession()->get('panier') ? $this->cartRepository->find($this->session->getSession()->get('panier')) : null);
		...
		if ($this->user && $this->user instanceof BaseUserShop) {
			...
			if ($cartSession) {
				...
				if ($ifCartExist) {
					...
					$this->session->getSession()->set('panier', $newCart->getId());
